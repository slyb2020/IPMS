...
class PropertyPanel(wx.Panel):
    def __init__(self, parent, log, size):
        ...
        #
        pg.AddPage("Page 1 - 系统参数设置")

        pg.Append(wxpg.PropertyCategory("1 - 系统基本运行参数"))
...
class SetupPropertyDialog(wx.Dialog):
    def __init__(self, parent, log, size=wx.DefaultSize, pos=wx.DefaultPosition,
        ...
        self.Create(parent, -1, "系统参数设置对话框", pos, size, style)
        sizer = wx.BoxSizer(wx.VERTICAL)
        self.propertyPanel = PropertyPanel(self, self.log, size=(1000, 800))
